...
export async function runLiveMonadDemo(options: LiveMonadDemoOptions = {}) {
  ...
  try {
    ...
    const rpcVerification = await verifyTransactionOnMonadRpc(
      env.rpcUrl,
      result.result.transactionHash as `0x${string}`
    );

    return {
      market: result.result.market,
      asset: result.result.asset,
      safetyMargin: result.result.safetyMargin,
      estimatedEdge: result.result.estimatedEdge,
      action: result.result.action,
      paymentMode: result.result.paymentMode,
      payerAddress: payer.address,
      payToAddress: env.payToAddress,
      quotedAmountAtomic: result.result.quotedAmountAtomic,
...
async function verifyTransactionOnMonadRpc(rpcUrl: string, hash: `0x${string}`) {
